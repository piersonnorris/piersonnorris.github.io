...
3. **`docs/ASSET_TRACKER_SPEC.md`** — no longer in this repo. The portfolio dashboard it specified moved to its own private repo (`stock-trackers`) on 2026-09-15, so there is no tracker to build here.
...
- **Security is non-negotiable.** Never ask me to paste secrets (API keys, the Google service-account JSON, PINs) into this chat, into any file in the repo, or into client-side code. Those live only in GitHub Actions Secrets. My real portfolio numbers never appear in this repo or the public build — the tracker lives in its own private repo. If I ever paste something sensitive by mistake, tell me immediately and how to rotate it.
